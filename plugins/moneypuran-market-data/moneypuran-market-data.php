...
<div class="mp-az" data-symbol="<?php echo esc_attr($a['symbol']); ?>">
  ...
  <div class="mp-az__top">
    ...
    <div class="mp-az__modes" role="tablist">
      <?php foreach (array('intraday' => 'Short-term', 'swing' => 'Swing', 'positional' => 'Positional') as $mk => $ml) : ?>
  ...
  <div class="mp-az__view mp-az__view--<?php echo $vc; ?>">
    <div class="mp-az__view-main">
      <span class="mp-az__view-eyebrow">MoneyPuran Market View &middot; <?php $mlbl = array('intraday' => 'Short-term', 'swing' => 'Swing', 'positional' => 'Positional'); echo esc_html($mlbl[$mode] ?? ucfirst($mode)); ?></span>
  ...
  <div class="mp-az__why">
    ...
    <?php endif; ?>
    <p class="mp-az__ai-src">Auto-generated from the computed indicators &mdash; price trend, momentum, volume, key levels and market context. Not a recommendation.</p>
  </div>
  <?php endif; ?>
...
/* ============================================================================
 * PROGRAMMATIC COMMODITY PAGES  (v1.26.0)
 *   /gold-rate/            national hub          /silver-rate/
 *   /gold-rate/<city>/     per-city page         /silver-rate/<city>/
 *
 *   - Real WP pages ("gold-rate" / "silver-rate") hold the hub; a rewrite rule
 *     maps /<hub>/<city>/ onto the same page with a `mp_city` query var, so the
 *     theme template, header/footer and Rank Math all behave normally.
 *   - 100% server-rendered: Googlebot receives the full price tables, 10-day
 *     history and FAQ in the initial HTML with no client JS required.
 *   - 15-minute edge cache (bullion transient already refreshes every ~2 min).
 *   - Dataset + BreadcrumbList + FAQPage JSON-LD in <head>.
 *   - 301 from the legacy /gold-rates/ and /silver-rate-today/ URLs.
 * ==========================================================================*/

/* slug => [Display, State/UT, local-market note] */
function mp_comm_cities() {
    return array(
        'delhi' => array('Delhi', 'Delhi (NCT)',
            'Delhi is one of India&rsquo;s largest bullion trading centres, so retail premiums over the pan-India reference rate tend to be thin. 24K and 22K are both widely sold.'),
        'mumbai' => array('Mumbai', 'Maharashtra',
            'Mumbai is home to the India Bullion &amp; Jewellers Association (IBJA), whose published rates most Indian jewellers use as their daily benchmark. Rates here track the national reference closely.'),
        'chennai' => array('Chennai', 'Tamil Nadu',
            'Chennai jewellers quote almost entirely in 22K and price per gram rather than per 10g. Local rates often carry a small regional premium and demand spikes sharply around Akshaya Tritiya and the wedding season.'),
        'kolkata' => array('Kolkata', 'West Bengal',
            'Kolkata is eastern India&rsquo;s bullion distribution hub. Demand skews towards lightweight 22K jewellery and hallmarked coins; rates broadly follow the IBJA reference.'),
        'bengaluru' => array('Bengaluru', 'Karnataka',
            'Like most of south India, Bengaluru retail is 22K-dominated and quoted per gram. Prices move with the national reference plus local making-charge conventions.'),
        'hyderabad' => array('Hyderabad', 'Telangana',
            'Hyderabad has strong festival and wedding demand for 22K gold and pearl-set jewellery. Retail rates follow the pan-India reference with a modest local premium.'),
        'pune' => array('Pune', 'Maharashtra',
            'Pune rates mirror Mumbai closely given the short distance to the IBJA benchmark market; both 24K investment bars and 22K jewellery are common.'),
        'ahmedabad' => array('Ahmedabad', 'Gujarat',
...
/* Current request context: [metal|'', citySlug|'', CityDisplay|'']
   Not statically cached - the queried object may not be ready on the first call. */
function mp_comm_ctx() {
    if (is_admin()) return array('', '', '');
    $obj = get_queried_object();
    $name = ($obj instanceof WP_Post) ? $obj->post_name : '';
...
function mp_comm_history($metal, $days = 10) {
    ...
    $bars = array_slice($bars, -($days + 1)); // +1 so we can show day-on-day delta
    $out = array();
    $prev = null;
    foreach ($bars as $b) {
        $close = (float) $b[4];
    ...
    return array_slice($out, -$days);
}

/* Full bundle for the shortcode + schema. Memoised per (metal,city) per request. */
function mp_comm_data($metal, $citySlug = '') {
    static $memo = array();
    $mk = $metal . '|' . $citySlug;
    if (array_key_exists($mk, $memo)) return $memo[$mk];
...
function mp_comm_faq($metal, $citySlug = '') {
    ...
    $faq[] = array('Why does the ' . $M . ' rate differ between Indian cities?',
        'GST on ' . $M . ' is a uniform 3% across India, so state-level differences are now small. Remaining variation comes from local jewellers&rsquo; association benchmarks, transport and insurance from bullion hubs, coin/bar premiums and local demand.');
    $faq[] = array('How is the ' . $M . ' rate in India calculated?',
        'The pan-India reference rate is derived from the international ' . $M . ' price (US dollars per troy ounce) converted to rupees per gram at the live USD/INR rate, then adjusted for India&rsquo;s import duty and 3% GST. Jewellers add making charges and hallmarking on top.');
    $faq[] = array('Is GST included in the ' . $M . ' rate shown here?',
        'The headline per-gram rate is the metal value only. The calculator on this page adds 3% GST and optional making charges so you can see the final billed amount.');
    $faq[] = array('What will ' . $M . ' prices be tomorrow?',
        'No one can reliably predict the next day&rsquo;s ' . $M . ' price. It moves with the international ' . $M . ' price, the US dollar, interest-rate expectations and safe-haven demand. This page updates through the day so you can track the trend.');
    return $faq;
}
...
<div class="mp-comm" data-metal="<?php echo esc_attr($metal); ?>" data-hub="<?php echo esc_attr($hub); ?>" data-city="<?php echo esc_attr($citySlug); ?>">
  ...
  <div class="mp-comm__disc">
    <strong>Disclaimer.</strong> MoneyPuran is a financial news and educational portal. The rates on this page are indicative, derived from international prices and the rupee exchange rate, and are <em>not</em> a retail quote or an offer to buy or sell. They exclude jeweller making charges and may be delayed. Confirm the live rate with a hallmarked jeweller before transacting. Nothing here is investment advice.
  </div>
</div>
...
/* /<hub>/<city>/ resolves the hub page - don't let WP canonical-redirect it away. */
...
add_action('template_redirect', function () {
    ...
    // 15-minute edge cache for these pages.
    if (function_exists('do_action')) do_action('litespeed_control_set_ttl', 900);
    header('Cache-Control: public, max-age=900, s-maxage=900, stale-while-revalidate=1800');
}, 9);
...
add_filter('the_title', function ($title, $post_id = 0) {
    ...
    $t = mp_comm_titles();
    if ($t && get_post_field('post_name', $post_id) === mp_comm_hub_slugs()[$t['metal']]) return $t['h1'];
    return $title;
}, 10, 2);
...
add_action('wp_head', function () {
    ...
    $crumbs = array(
        ...
        array('@type' => 'ListItem', 'position' => 2, 'name' => 'Commodities', 'item' => home_url('/category/commodities/')),
        array('@type' => 'ListItem', 'position' => 3, 'name' => $M . ' Rate', 'item' => home_url('/' . $hub . '/')),
    );
    if ($citySlug) $crumbs[] = array('@type' => 'ListItem', 'position' => 4, 'name' => $cityDisplay, 'item' => $t['canon']);
    ...
    $faqPage = array('@context' => 'https://schema.org', '@type' => 'FAQPage', 'mainEntity' => $faqItems);

    echo "\n<script type=\"application/ld+json\">" . wp_json_encode($dataset, JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE) . "</script>\n";
    echo '<script type="application/ld+json">' . wp_json_encode($breadcrumb, JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE) . "</script>\n";
    echo '<script type="application/ld+json">' . wp_json_encode($faqPage, JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE) . "</script>\n";
}, 3);
...
add_action('init', function () {
    ...
    foreach (array(
        ...
        if ($existing) {
            if (strpos((string) $existing->post_content, 'mp_commodity_page') === false) {
                wp_update_post(array('ID' => $existing->ID, 'post_content' => $meta[1]));
            }
            continue;
        }
...
add_filter('rank_math/sitemap/index', function ($links) {
    $u = home_url('/commodities-sitemap.xml');
    if (strpos($links, $u) === false) {
        $links .= '<sitemap><loc>' . esc_url($u) . '</loc><lastmod>' . gmdate('c') . '</lastmod></sitemap>' . "\n";
    }
    return $links;
});
...
add_action('mp_md_cron_groups', function () {
    $last = (float) get_option('mp_comm_indexnow_px', 0);
    $grp  = get_transient(MP_MD_GRP_KEY);
    $now  = (is_array($grp) && !empty($grp['bullion_inr']['gold_24k_10g'])) ? (float) $grp['bullion_inr']['gold_24k_10g'] : 0;
    if (!$now) return;
    if (!$last || abs($now - $last) / $last > 0.004) { // >0.4% move since last ping
        mp_comm_indexnow(mp_comm_all_urls());
        update_option('mp_comm_indexnow_px', $now, false);
    }
}, 20);

/* ---- styles --------------------------------------------------------- */
add_action('wp_head', function () {
    if (!mp_comm_titles()) return;
    ?>
...
/* ============================================================================
 * E-E-A-T + TRUST  (v1.28.0)  [Task 4 + Task 3 author schema]
 *   - Real author profiles (honest bios, job titles, beats) - no fabricated
 *     credentials or LinkedIn URLs (filterable so real ones can be added).
 *   - Person / ProfilePage schema on author archives + a profile card.
 *   - Auto-assign the beat author on publish (covers the newsroom worker).
 *   - Standardised SEBI disclaimer box on every analysis page.
 *   - "Intraday" -> "Short-term"; "AI Market View" -> "MoneyPuran Market View".
 * ==========================================================================*/

/* slug => [display, job title, beat category slugs, honest bio] */
function mp_eeat_people() {
    return apply_filters('mp_eeat_people', array(
        'diksha-kumari' => array(
            'Diksha Kumari', 'Markets Writer',
            array('indian-markets', 'us-markets', 'global-markets', 'economy', 'central-banks', 'regulation', 'markets'),
            'Diksha Kumari writes MoneyPuran&rsquo;s daily markets coverage &mdash; the Sensex and Nifty, sector performance, FII and DII flows, the rupee and the global cues that move Indian equities. She focuses on explaining what moved and why in plain language, without tips or price targets.',
        ),
        'sumant' => array(
            'Sumant Kumar', 'Commodities Writer',
            array('commodities'),
        ...
        'deepak' => array(
            'Deepak Chauhan', 'Personal Finance Writer',
            array('personal-finance', 'business'),
            'Deepak Chauhan writes MoneyPuran&rsquo;s personal-finance explainers &mdash; mutual funds, tax-saving instruments, insurance and the fundamentals of investing for Indian readers.',
        ),
        'dipu' => array(
            'Dipu Kumar', 'Editor',
            array(),
            'Dipu Kumar is MoneyPuran&rsquo;s editor. He reviews the site&rsquo;s markets, commodities and personal-finance coverage for accuracy and clarity and is responsible for its editorial standards and corrections policy.',
        ),
    ));
}
function mp_eeat_user_id($slug) {
    $u = get_user_by('slug', $slug);
    return $u ? (int) $u->ID : 0;
}
/* category slug -> author user id (first person whose beat lists it) */
function mp_eeat_cat_author($catSlug) {
    foreach (mp_eeat_people() as $slug => $p) {
        if (in_array($catSlug, $p[2], true)) return mp_eeat_user_id($slug);
    }
    return 0;
}
function mp_eeat_linkedin($slug) {
    return (string) apply_filters('mp_eeat_linkedin', '', $slug); // supply real URLs here later
}

/* ---- one-time: write bios / names / job titles --------------------- */
add_action('init', function () {
    if (get_option('mp_eeat_profiles_v') === '2') return;
    if (wp_doing_ajax() || wp_doing_cron() || (defined('REST_REQUEST') && REST_REQUEST)) return;
    foreach (mp_eeat_people() as $slug => $p) {
        $uid = mp_eeat_user_id($slug);
        if (!$uid) continue;
        $parts = explode(' ', $p[0], 2);
        wp_update_user(array(
            ...
            'display_name' => $p[0],
            'nickname' => $p[0],
            'description' => wp_strip_all_tags($p[3]),
        ));
        update_user_meta($uid, 'mp_job_title', $p[1]);
    }
    update_option('mp_eeat_profiles_v', '2');
}, 32);

/* ---- auto-assign the beat author on publish ------------------------ */
add_action('transition_post_status', function ($new, $old, $post) {
    static $busy = false;
    if ($busy) return;
    if ($new !== 'publish' || !($post instanceof WP_Post) || $post->post_type !== 'post') return;
    if (wp_is_post_revision($post) || wp_is_post_autosave($post)) return;
    $cur = (int) $post->post_author;
    // only reassign generic/admin authorship, never a hand-picked beat author
    $beatIds = array_filter(array_map('mp_eeat_user_id', array_keys(mp_eeat_people())));
    if (in_array($cur, $beatIds, true)) return;
    $cats = wp_get_post_categories($post->ID, array('fields' => 'all'));
    $target = 0;
    foreach ((array) $cats as $c) { if (is_object($c)) { $target = mp_eeat_cat_author($c->slug); if ($target) break; } }
    if (!$target) $target = mp_eeat_user_id('diksha-kumari'); // default = markets
    if ($target && $target !== $cur) {
        $busy = true;
        wp_update_post(array('ID' => $post->ID, 'post_author' => $target));
        $busy = false;
    }
}, 20, 3);

/* ---- author archive: profile card + Person/ProfilePage schema ----- */
function mp_eeat_author_card($uid) {
    $u = get_userdata($uid);
    if (!$u) return '';
    $title = get_user_meta($uid, 'mp_job_title', true);
    $bio   = $u->description;
    $li    = mp_eeat_linkedin($u->user_nicename);
    ob_start(); ?>
<div class="mp-authcard">
  <?php echo get_avatar($uid, 72, '', $u->display_name, array('class' => 'mp-authcard__img')); ?>
  <div class="mp-authcard__body">
    <p class="mp-authcard__role"><?php echo esc_html($title ?: 'Writer'); ?> &middot; MoneyPuran</p>
    <?php if ($bio) : ?><p class="mp-authcard__bio"><?php echo esc_html($bio); ?></p><?php endif; ?>
    <p class="mp-authcard__links">
      <a href="<?php echo esc_url(get_author_posts_url($uid)); ?>">All articles</a>
      <?php if ($li) : ?> &middot; <a href="<?php echo esc_url($li); ?>" rel="noopener nofollow" target="_blank">LinkedIn</a><?php endif; ?>
      &middot; <a href="<?php echo esc_url(home_url('/editorial-policy/')); ?>">Editorial standards</a>
    </p>
  </div>
</div>
<style>
.mp-authcard{display:flex;gap:14px;align-items:flex-start;border:1px solid var(--mp-border,#e2e8f0);border-radius:12px;padding:14px 16px;margin:14px 0 22px;background:var(--mp-surface2,#f8fafc)}
.mp-authcard__img{border-radius:50%;flex:0 0 auto}
.mp-authcard__role{margin:0 0 4px;font-size:12px;text-transform:uppercase;letter-spacing:.03em;color:var(--mp-muted,#64748b)}
.mp-authcard__bio{margin:0 0 6px;font-size:14px;line-height:1.55}
.mp-authcard__links{margin:0;font-size:13px}
html[data-theme="dark"] .mp-authcard{background:#111827;border-color:rgba(255,255,255,.08)}
</style>
    <?php
    return ob_get_clean();
}
add_action('mp_after_archive_title', function () {
    if (is_author()) echo mp_eeat_author_card(get_queried_object_id());
});
...
add_action('wp_head', function () {
    ...
    $person = array(
        '@type' => 'Person',
        '@id'   => get_author_posts_url($uid) . '#person',
        'name'  => $u->display_name,
        'url'   => get_author_posts_url($uid),
        'jobTitle' => get_user_meta($uid, 'mp_job_title', true) ?: 'Writer',
        'worksFor' => array('@type' => 'Organization', 'name' => 'MoneyPuran', 'url' => home_url('/')),
        'description' => $u->description,
        'knowsAbout' => array('Indian stock market', 'Sensex and Nifty', 'Gold and silver prices', 'Personal finance', 'Investing'),
    );
    $li = mp_eeat_linkedin($u->user_nicename);
    if ($li) $person['sameAs'] = array($li);
    $node = array(
...
/* ---- standardised SEBI disclaimer box on analysis pages ----------- */
function mp_sebi_box() {
    return '<div class="mp-sebi-box" role="note"><strong>Disclaimer:</strong> MoneyPuran is a financial news and educational portal. '
         . 'Not SEBI-registered investment advice. Nothing here is a recommendation to buy or sell any security; '
         . 'figures may be delayed. Consult a SEBI-registered investment adviser before acting.'
         . '<style>.mp-sebi-box{border:1px solid var(--mp-border,#e2e8f0);border-left:3px solid #d97706;border-radius:8px;'
         . 'padding:11px 14px;margin:20px 0;font-size:12.5px;line-height:1.55;color:var(--mp-muted,#64748b);background:var(--mp-surface2,#f8fafc)}'
         . 'html[data-theme="dark"] .mp-sebi-box{background:#111827;border-color:rgba(255,255,255,.08);border-left-color:#d97706}</style></div>';
}
add_filter('the_content', function ($html) {
    if (is_admin() || !is_singular() || is_feed()) return $html;
    if (strpos($html, 'mp-sebi-box') !== false) return $html;
    $triggers = array('mp-az', 'mp-why', 'mp-usm', 'mp-scr', 'mp-stk', 'mp-sector');
    foreach ($triggers as $t) {
        if (strpos($html, $t) !== false) return $html . mp_sebi_box();
    }
    return $html;
}, 20);
